...
export function importFromExcel(fileBuffer) {
  ...
  const mapRow = (r) => ({
    ...
    price_with_tax: Number(r['含税价']??r['price_with_tax']??null)||null,
    price_without_tax: Number(r['未税价']??r['price_without_tax']??null)||null,
...
export function importSamplesFromExcel(fileBuffer) {
  ...
  const mapRow = (r) => ({
    ...
    temperature: String(r['温度']??r['temperature']??''),
    price_with_tax: Number(r['含税价']??r['price_with_tax']??null)||null,
    cost_price: Number(r['本价含税']??r['本价']??r['cost_price']??null)||null,
