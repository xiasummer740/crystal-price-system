...
async function handleDelete(item){try{await showConfirmDialog({title:'确认删除',message:`删除物料「${item.material_code||item.material_name}」的报价？`});await store.remove(item.id);showToast('已删除');reload()}catch{}}
let resizing=null
